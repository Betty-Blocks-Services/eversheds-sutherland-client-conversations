(() => ({
  ...
  jsx: (() => {
    ...
    useEffect(() => {
      B.defineFunction('setFilterTopic', (v) => {
        const filter = v ? { categories: { id: { eq: v } } } : undefined;
        setFilterTopic(filter);
      });
      B.defineFunction('setFilterProduct', (v) => {
        const filter = v ? { products: { id: { eq: v } } } : undefined;
        setFilterProduct(filter);
      });
      B.defineFunction('setFilterSector', (v) => {
        const filter = v ? { sectors: { id: { eq: v } } } : undefined;
        setFilterSector(filter);
      });

      B.defineFunction('setFilterSearch', (v) => {
        const filter = v ? { title: { matches: v } } : undefined;
    ...
    }, [filterSector, filterTopic, filterProduct, filterSearch]);
